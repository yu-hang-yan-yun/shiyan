...
<template>
  <div>
   <div class="v-nav">
      <el-button type="info" size="mini" @click="getback" round>返回</el-button>
      <el-button type="success" icon="el-icon-check" @click="submission" size="mini" round>保存提交</el-button>
    </div>
    <div class="ace-container">
      ...
      <div class="bookmarklet" @click="toggleConfigPanel" v-show="false"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
   getback() {
      this.$router.push({ name: "JavaAdmin" });
    },
    submission() {
      this.$router.push({ name: "JavaAdmin" });
    },
    ...
    change() {
      this.$emit("input", this.aceEditor.getSession().getValue(this.value));
    },
...
.bookmarklet {
  ...
  border-width: 9px;
  border-style: solid;
  border-color: lightblue rgb(129, 185, 202) rgb(55, 212, 218)
    rgb(206, 173, 230);
