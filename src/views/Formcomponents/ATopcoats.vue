...
export default {
  ...
  methods: {
      ...
      if (isNeedArr.length && originalArr.length) {
        for (let i = 0; i < isNeedArr.length; i++) {
          for (let k = 0; k < originalArr.length; k++) {
            if (isNeedArr[i]["chargeNumbel"] === originalArr[k]["chargeNumbel"]) {
    ...
    okSubmit() {
      this.$store.dispatch("setAdminName", this.chargeNumbel);
      this.$router.push({ name: "Item", params: { AdminEdit: true } });
